...
test("ensureDefaultCommonBaseInstructions는 값이 비어 있어도 기본 일반지침을 반환한다", () => {
  ...
  assert.equal(
    DEFAULT_COMMON_BASE_INSTRUCTIONS.includes(DEFAULT_USAGE_ROLLOVER_INSTRUCTION_SECTION),
    true
  );
  assert.equal(
    DEFAULT_COMMON_BASE_INSTRUCTIONS.includes("각 섹션 제목 앞뒤로 한 줄씩 비우고, 섹션 안의 항목 사이에도 한 줄 공백을 넣어 사용자 친화적으로 구분하십시오."),
    true
  );
  assert.equal(
    DEFAULT_COMMON_BASE_INSTRUCTIONS.includes("해당 제목은 한 번만 쓰고, 같은 제목이나 접두어를 반복하지 않습니다."),
    true
  );
});
...
test("ensureDefaultCommonBaseInstructions는 구버전 기본 지침만 저장돼 있어도 새 포맷 섹션을 추가한다", () => {
  ...
  assert.equal(
    resolved.includes(DEFAULT_USAGE_ROLLOVER_INSTRUCTION_SECTION),
    true
  );
  assert.equal(resolved.includes("[목표]"), true);
  assert.equal(resolved.includes("[계획]"), true);
  assert.equal(resolved.includes("[진행 내역]"), true);
  assert.equal(resolved.includes("[최종 보고]"), true);
});

test("ensureDefaultCommonBaseInstructions는 구버전 120% 문구를 100% 새 쓰레드 지침으로 치환한다", () => {
  const existing = [
    "사용자 맞춤 일반지침",
    LEGACY_USAGE_ROLLOVER_INSTRUCTION_SECTION
  ].join("\n\n");
  const resolved = ensureDefaultCommonBaseInstructions(existing);

  assert.equal(resolved.includes(LEGACY_USAGE_ROLLOVER_INSTRUCTION_SECTION), false);
  assert.equal(resolved.includes(DEFAULT_USAGE_ROLLOVER_INSTRUCTION_SECTION), true);
  assert.match(resolved, /사용자 맞춤 일반지침/);
});

test("ensureDefaultCommonBaseInstructions는 기존 진행 내역 섹션이 있어도 누락된 반복 방지 문구만 보강한다", () => {
  const existing = [
    "사용자 맞춤 일반지침",
  ...
  assert.equal(
    resolved.includes("- 해당 제목은 한 번만 쓰고, 같은 제목이나 접두어를 반복하지 않습니다."),
    true
  );
  assert.equal(
    resolved
      .split("\n")
      .filter((line) => line.trim() === "[진행 내역]").length,
    1
  );
});
