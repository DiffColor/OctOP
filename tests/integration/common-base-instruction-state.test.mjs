...
test("ensureDefaultCommonBaseInstructions는 값이 비어 있어도 기본 일반지침을 반환한다", () => {
  ...
  assert.equal(
    DEFAULT_COMMON_BASE_INSTRUCTIONS.includes("응답이 길어져서 사용량을 120%를 넘어서는 경우에는 작업을 중단하고 현재 작업을 요약해서 다음 이슈로 넘길 수 있도록 정리하십시오."),
    true
  );
  assert.equal(
...
test("ensureDefaultCommonBaseInstructions는 구버전 기본 지침만 저장돼 있어도 새 포맷 섹션을 추가한다", () => {
  ...
  assert.equal(
    resolved.includes("응답이 길어져서 사용량을 120%를 넘어서는 경우에는 작업을 중단하고 현재 작업을 요약해서 다음 이슈로 넘길 수 있도록 정리하십시오."),
    true
  );
