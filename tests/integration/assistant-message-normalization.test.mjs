...
test("normalizeAssistantMessageContent는 반복된 목표와 계획 섹션을 임의로 제거하지 않는다", () => {
  const input = [
    "[목표]",
    "- 최초 목표",
    "",
  ...
  assert.equal(normalized.split("\n").filter((line) => line.trim() === "[목표]").length, 2);
  assert.equal(normalized.split("\n").filter((line) => line.trim() === "[계획]").length, 2);
  assert.equal(normalized.includes("- 최초 목표"), true);
  assert.equal(normalized.includes("- 중복 목표"), true);
  assert.equal(normalized.includes("- 영향 범위: 최초 범위"), true);
  assert.equal(normalized.includes("- 영향 범위: 중복 범위"), true);
  assert.equal(normalized.includes("- 실제 진행"), true);
});

test("normalizeAssistantMessageContent는 반복된 최종 보고 섹션도 임의로 제거하지 않는다", () => {
  const input = [
    "[최종 보고]",
    "- 변경 사항: 첫 보고",
    "",
  ...
  assert.equal(normalized.split("\n").filter((line) => line.trim() === "[최종 보고]").length, 2);
  assert.equal(normalized.includes("- 변경 사항: 첫 보고"), true);
  assert.equal(normalized.includes("- 변경 사항: 중복 보고"), true);
