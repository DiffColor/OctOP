...
test("stale degraded thread가 있으면 다른 thread issue는 queued로 유지된다", { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    await waitFor(async () => {
      ...
      const health = await bridge.request("/health");
      assert.equal(issueDetail.issue?.status, "queued");
      assert.equal(health.status?.app_server?.idle, false);
      return { issueDetail, health };
    }, {
      timeoutMs: 15000,
      intervalMs: 250,
      label: "other thread stays queued while degraded thread exists"
    });
  } catch (error) {
    error.message = `${error.message}\n\n[bridge stdout]\n${bridge.debugOutput().stdout}\n[bridge stderr]\n${bridge.debugOutput().stderr}`;
    throw error;
...
test("다른 thread issue 시작은 현재 running issue가 끝날 때까지 queued로 유지된다", { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    assert.equal(startPayload.blocked_by_thread_id, runningScenario.rootThreadId);
    ...
    await waitFor(async () => {
      ...
      assert.equal(secondaryIssue.issue?.status, "queued");
      assert.equal(fakeAppServer.getRequests("turn/start").length, 1);
      return { primaryIssue, secondaryIssue };
    }, {
      timeoutMs: 5000,
      intervalMs: 250,
      label: "secondary thread remains queued while primary thread runs"
    });

    completeIssueOnThread(fakeAppServer, {
      codexThreadId: runningScenario.sourceCodexThreadId,
      turnId: "turn-primary-cross-thread-completed"
    });
    ...
    await waitFor(async () => {
      ...
      timeoutMs: 45000,
      intervalMs: 300,
      label: "secondary thread starts after primary thread completion"
    });
  } catch (error) {
    error.message = `${error.message}\n\n[bridge stdout]\n${bridge.debugOutput().stdout}\n[bridge stderr]\n${bridge.debugOutput().stderr}`;
    throw error;
...
test("thread not found stop error 후 manual_refresh recovery 뒤에도 같은 thread는 재시작되고 새 thread는 queued로 유지된다", { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    assert.equal(secondStartPayload.blocked_by_thread_id, rootThreadId);

    await waitFor(async () => {
      const issuePayload = await bridge.request(`/api/issues/${secondIssueId}`);
      assert.equal(issuePayload.issue?.status, "queued");
      return issuePayload;
    }, {
      timeoutMs: 15000,
      intervalMs: 250,
      label: "new thread issue remains queued while restarted thread runs"
    });
  } catch (error) {
    error.message = `${error.message}\n\n[bridge stdout]\n${bridge.debugOutput().stdout}\n[bridge stderr]\n${bridge.debugOutput().stderr}`;
    throw error;
