...
test("브리지 app-server running issue heartbeat timeout 중에도 즉시 재연결하지 않고 연결을 유지한다", { timeout: 90000 }, async (t) => {
  const homeDir = await mkdtemp(join(tmpdir(), "octop-running-heartbeat-stable-int-"));
  const fakeAppServer = new FakeAppServer({
    ignorePongCount: 4
  });
  const appServerUrl = await fakeAppServer.start();
  ...
  const bridge = new BridgeProcess({
    port: bridgePort,
    token: "octop-running-heartbeat-stable-token",
    userId: "running-heartbeat-stable-user",
    bridgeId: `running-heartbeat-stable-${randomUUID().slice(0, 8)}`,
    homeDir,
    appServerUrl,
    extraEnv: {
      OCTOP_APP_SERVER_HEARTBEAT_INTERVAL_MS: "250",
  ...
  const scenario = await createRunningIssueScenario(bridge, {
    project,
    threadName: "Running heartbeat stable"
  });
...
test("연속 무진전 running backfill도 websocket을 강제 재연결하지 않고 degraded로 유지한다", { timeout: 120000 }, async (t) => {
  const homeDir = await mkdtemp(join(tmpdir(), "octop-running-backfill-stable-int-"));
  const fakeAppServer = new FakeAppServer();
  fakeAppServer.options.onTurnStart = ({ server, threadId }) => {
    server.notify("item/agentMessage/delta", {
      threadId,
  ...
  const bridge = new BridgeProcess({
    port: bridgePort,
    token: "octop-running-backfill-stable-token",
    userId: "integration-user",
    bridgeId: `running-backfill-stable-${randomUUID().slice(0, 8)}`,
    homeDir,
    appServerUrl,
    extraEnv: {
      OCTOP_RUNNING_ISSUE_WATCHDOG_INTERVAL_MS: "200",
  ...
  try {
    ...
    const { rootThreadId, activeIssueId } = await createRunningIssueScenario(bridge, {
      project,
      threadName: "Running Backfill Stable"
    });
    ...
    await waitFor(async () => {
      ...
      timeoutMs: 20000,
      intervalMs: 250,
      label: "running backfill stays degraded without forced reconnect"
    });
  } catch (error) {
    error.message = `${error.message}\n\n[bridge stdout]\n${bridge.debugOutput().stdout}\n[bridge stderr]\n${bridge.debugOutput().stderr}`;
    throw error;
