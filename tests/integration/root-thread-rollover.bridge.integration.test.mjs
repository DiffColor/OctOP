...
test("thread/list 누락이 반복되면 stale running issue를 failed 처리한다", { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    await waitFor(async () => {
      ...
      assert.equal(issueDetail.issue?.last_event, "watchdog.failed");
      assert.equal(continuity.active_physical_thread?.codex_thread_id ?? null, null);
      assert.equal(continuity.active_physical_thread?.status, "failed");
      assert.equal(continuity.active_physical_thread?.turn_id ?? null, null);
      assert.equal(health.status?.app_server?.initialized, true);
      return { issueDetail, continuity, health };
    }, {
      timeoutMs: 5000,
      intervalMs: 250,
      label: "running issue failed after missing remote thread"
    });
    ...
    await waitFor(async () => {
      const payload = await bridge.request(`/api/issues/${activeIssueId}`);
      assert.equal(payload.issue?.status, "failed");
      return payload;
    }, {
      timeoutMs: 45000,
      intervalMs: 300,
      label: "issue remains failed after late completion event"
    });
  } catch (error) {
    error.message = `${error.message}\n\n[bridge stdout]\n${bridge.debugOutput().stdout}\n[bridge stderr]\n${bridge.debugOutput().stderr}`;
    throw error;
