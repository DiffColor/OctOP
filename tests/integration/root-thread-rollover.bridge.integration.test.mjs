...
test("프로젝트 일반지침과 전역/쓰레드별 개발지침이 초기 실행과 rollover handoff 프롬프트 모두에 적용된다", { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    assert.equal("projectDeveloperInstructions" in (initialThreadStartRequest?.params ?? {}), false);
    assert.equal("threadDeveloperInstructions" in (initialThreadStartRequest?.params ?? {}), false);
    ...
    assert.equal(initialInput.includes("[프로젝트 일반지침]"), false);
    assert.equal(initialInput.includes("[프로젝트 개발지침]"), false);
    assert.equal(initialInput.includes("[쓰레드 개발지침]"), false);
    ...
    assert.equal("projectDeveloperInstructions" in (rolloverThreadStartRequest?.params ?? {}), false);
    assert.equal("threadDeveloperInstructions" in (rolloverThreadStartRequest?.params ?? {}), false);
    ...
    assert.equal(rolloverInput.includes("[프로젝트 일반지침]"), false);
    assert.equal(rolloverInput.includes("[프로젝트 개발지침]"), false);
    assert.equal(rolloverInput.includes("[쓰레드 개발지침]"), false);
