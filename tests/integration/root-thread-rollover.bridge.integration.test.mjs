...
test("stale degraded thread가 있어도 다른 thread issue 시작은 막히지 않는다", { timeout: 120000 }, async (t) => {
  ...
  try {
    ...
    const nextScenario = await createRunningIssueScenario(bridge, {
      project,
      threadName: "Follower Running Thread"
    });

    await waitFor(async () => {
      const issueDetail = await bridge.request(`/api/issues/${nextScenario.activeIssueId}`);
      const health = await bridge.request("/health");
      assert.equal(issueDetail.issue?.status, "running");
      assert.equal(health.status?.app_server?.idle, false);
      return { issueDetail, health };
      ...
      timeoutMs: 15000,
      intervalMs: 250,
      label: "other thread can still start while degraded thread exists"
    });
  } catch (error) {
    error.message = `${error.message}\n\n[bridge stdout]\n${bridge.debugOutput().stdout}\n[bridge stderr]\n${bridge.debugOutput().stderr}`;
    throw error;
