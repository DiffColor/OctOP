...
- `apps/android-watch`
  Wear OS 기반 워치 앱입니다. 워치 푸시 토큰을 OctOP gateway에 등록하고 워치에서 OctOP 푸시를 수신합니다.
- `apps/apple-watch`
  watchOS 기반 워치 앱 소스입니다. 워치 푸시 토큰을 OctOP gateway에 등록하고 워치에서 OctOP 푸시를 수신합니다.
- `apps/api`
...
- OctOP push gateway를 통한 Wear OS / watchOS 워치 토큰 등록 경로
- OctOP push gateway 내부의 Web Push / FCM / APNs 하위 전송 경로
...
- `OCTOP_PUSH_FCM_*`, `OCTOP_PUSH_APNS_*`
  OctOP gateway가 워치 푸시를 각 플랫폼 전달망으로 넘길 때 사용하는 서버측 자격 증명입니다. 워치 앱은 이 값 대신 gateway에 토큰만 등록합니다.
- `OCTOP_BRIDGE_HOST`
- `OCTOP_BRIDGE_PORT`
- `OCTOP_BRIDGE_TOKEN`
- `OCTOP_BRIDGE_MODE`
