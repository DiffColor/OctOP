...
async function seedStandaloneWorkspace(page, { activeView = "inbox", viewport = null } = {}) {
  if (viewport) {
  ...
  await page.addInitScript(
    ({ sessionKey, workspaceLayoutKey, workspaceSnapshotKey, loginId, bridgeId, projectId, threadId, nextActiveView }) => {
      const session = {
...
async function mockStandaloneApiRoutes(page) {
...
test.describe("모바일 PWA standalone 히스토리 안정성", () => {
  ...
  test("wide split 쓰레드리스트에서는 뒤로 가기 시 종료 팝업이 열린다", async ({ page }) => {
    await seedStandaloneWorkspace(page, { activeView: "inbox" });
    await mockStandaloneApiRoutes(page);
  ...
  test("좁은 채팅 상세에서는 뒤로 가기로 리스트로 이동하고 다시 누르면 종료 팝업이 열리며 한 번 더 누르면 닫힌다", async ({
    page
  }) => {
    await seedStandaloneWorkspace(page, {
      activeView: "thread",
      viewport: {
        width: 430,
        height: 932
    ...
    await page.goto(baseUrl, { waitUntil: "load" });
    await expect(page.getByTestId("thread-detail-header-filters")).toBeVisible();
    await expect(page.getByTestId(`thread-list-item-${THREAD_ID}`)).toHaveCount(0);
    ...
    await expect(page.getByRole("heading", { name: "Standalone Project" })).toBeVisible();
    await expect(page.getByTestId(`thread-list-item-${THREAD_ID}`)).toBeVisible();
    await expect(page.getByTestId("thread-detail-header-filters")).toHaveCount(0);
