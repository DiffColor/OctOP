...
test('stt 전용 모드에서는 잠시 쉬었다가 새 final result가 와도 이전 발화를 지우지 않는다', async ({ page }) => {
  await seedMobileSession(page);
  ...
  await page.evaluate(() => {
    window.__voiceTest.emitSpeechRecognitionResult({
      resultIndex: 0,
      results: [{ transcript: '안녕하세요', isFinal: true }]
    });
  });

  await expect(promptInput).toHaveValue('안녕하세요');
  ...
  await page.evaluate(() => {
    window.__voiceTest.emitSpeechRecognitionResult({
      resultIndex: 0,
      results: [{ transcript: '123', isFinal: true }]
    });
  });

  await expect(promptInput).toHaveValue('안녕하세요 123');
  await expect(promptInput).not.toHaveValue('123');
});
...
test('stt 전용 모드에서는 같은 발화가 여러 final index에 누적 확장돼도 마지막 확정 문장만 남긴다', async ({ page }) => {
  await seedMobileSession(page);
  await seedVoiceCapabilitySnapshot(page, {
    realtime: 'blocked',
    tts: 'blocked',
  ...
  await page.evaluate(() => {
    window.__voiceTest.emitSpeechRecognitionResult({
      resultIndex: 0,
      results: [
        { transcript: '중복이 좀', isFinal: true },
        { transcript: '중복이 좀 처리', isFinal: true },
        { transcript: '중복이 좀 처리 안 돼', isFinal: true }
      ]
    });
  });

  await expect(promptInput).toHaveValue('중복이 좀 처리 안 돼');
  await expect(promptInput).not.toHaveValue('중복이 좀 중복이 좀 처리 중복이 좀 처리 안 돼');
});

test('stt 전용 모드에서는 이전 transcript 끝과 새 final 시작이 겹치면 prefix만 제거하고 앞 내용은 유지한다', async ({ page }) => {
  await seedMobileSession(page);
  await seedVoiceCapabilitySnapshot(page, {
    realtime: 'blocked',
    tts: 'blocked',
  ...
  await page.evaluate(() => {
    window.__voiceTest.emitSpeechRecognitionResult({
      resultIndex: 0,
      results: [
        { transcript: '이전 내용은 남기고 좋은 거', isFinal: true }
      ]
    });
  });

  await expect(promptInput).toHaveValue('이전 내용은 남기고 좋은 거');
  ...
  await page.evaluate(() => {
    window.__voiceTest.emitSpeechRecognitionResult({
      ...
      results: [
        { transcript: '이전 내용은 남기고 좋은 거', isFinal: true },
        { transcript: '좋은 거 프리픽스만 제거하라고', isFinal: true }
      ]
    });
  });

  await expect(promptInput).toHaveValue('이전 내용은 남기고 좋은 거 프리픽스만 제거하라고');
  await expect(promptInput).not.toHaveValue('이전 내용은 남기고 좋은 거 좋은 거 프리픽스만 제거하라고');
});
